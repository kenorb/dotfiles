...
def calc_avg(time1, time2):
    ...
    total_avg = (total_hs1 + total_hs2) / 2
    avg_mm = int(total_avg / (60*100))
    avg_ss = int(total_avg / (100)) - (avg_mm*60)
    avg_hs = int(total_avg % 100)
    return "%02d:%02d:%02d" % (avg_mm, avg_ss, avg_hs) 
